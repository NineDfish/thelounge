...
request.get(
	...
	(error, response, emojiStrategy) => {
		...
		for (const key in emojiStrategy) {
			if (emojiStrategy.hasOwnProperty(key)) {
				...
				for (let alternative of emojiStrategy[key].aliases.split("|")) {
					...
					if (
						fuzzy.test(shortname, alternative) ||
						fuzzy.test(alternative, shortname)
					) {
						continue;
					}
		...
		const fullNameEmojiMapOutput =
			JSON.stringify(fullNameEmojiMap, null, 2) + "\n";

		fs.writeFileSync(
			path.resolve(
				path.join(__dirname, "..", "client", "js", "libs", "simplemap.json")
			),
			emojiMapOutput
		);
		...
		fs.writeFileSync(
			path.resolve(
				path.join(__dirname, "..", "client", "js", "libs", "fullnamemap.json")
			),
