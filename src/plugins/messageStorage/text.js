...
class TextFileMessageStorage {
	...
	index(network, channel, msg) {
		...
		switch (msg.type) {
			...
			case Msg.Type.KICK:
				...
				line += `*** ${msg.target.nick} was kicked by ${msg.from.nick} (${
					msg.text
				})`;
				break;
			...
			case Msg.Type.CHGHOST:
				...
				line += `*** ${msg.from.nick} changed host to '${msg.new_ident}@${
					msg.new_host
				}'`;
				break;
			case Msg.Type.TOPIC:
		...
		fs.appendFile(
			path.join(logPath, `${cleanFilename(channel.name)}.log`),
			line,
			(e) => {
				if (e) {
	...
	static getNetworkFolderName(network) {
		...
		const networkName = cleanFilename(
			network.name.substring(0, 23).replace(/ /g, "-")
		);
