...
Helper.config = require(path.resolve(
...
function setHome(newPath) {
	...
	if (["example", "crypto", "zenburn"].includes(this.config.theme)) {
		if (this.config.theme === "example") {
			log.warn(
				`The default theme ${colors.red(
					"example"
				)} was renamed to ${colors.green("default")} as of The Lounge v3.`
			);
		} else {
			log.warn(
		...
		log.warn(
			`Falling back to theme ${colors.green(
				"default"
...
function getDefaultNick() {
	...
	return this.config.defaults.nick.replace(/%/g, () =>
...
function mergeConfig(oldConfig, newConfig) {
	return _.mergeWith(oldConfig, newConfig, (objValue, srcValue, key) => {
		...
		if (
			...
			log.warn(
				`Incorrect type for "${colors.bold(key)}", please verify your config.`
			);
