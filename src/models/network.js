...
Network.prototype.createIrcFramework = function(client) {
	...
	if (
		client.config.log &&
		client.messageStorage.find((s) => s.canProvideMessages())
	) {
...
Network.prototype.createWebIrc = function(client) {
	if (
		...
		!Helper.config.webirc.hasOwnProperty(this.host)
	) {
		return null;
	}
	...
	if (!this.ip) {
		log.warn(
			`Cannot find a valid WEBIRC configuration for ${this.nick}!${
				this.username
			}@${this.host}`
		);
...
Network.prototype.getNetworkStatus = function() {
	...
	if (this.irc && this.irc.connection && this.irc.connection.transport) {
		...
		if (transport.socket) {
			...
			const isAuthorized =
				transport.socket.encrypted && transport.socket.authorized;
...
Network.prototype.addChannel = function(newChan) {
	...
	if (newChan.type === Chan.Type.CHANNEL || newChan.type === Chan.Type.QUERY) {
		...
		for (let i = 1; i < this.channels.length; i++) {
			...
			if (
				newChan.name.localeCompare(compareChan.name, {sensitivity: "base"}) <=
					0 ||
				(compareChan.type !== Chan.Type.CHANNEL &&
					compareChan.type !== Chan.Type.QUERY)
			) {
				index = i;
				break;
			}
...
Network.prototype.export = function() {
	...
	network.channels = this.channels
		.filter(function(channel) {
			return (
				channel.type === Chan.Type.CHANNEL || channel.type === Chan.Type.QUERY
			);
		})
		.map(function(chan) {
			const keys = ["name"];
