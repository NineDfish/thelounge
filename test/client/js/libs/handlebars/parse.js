...
function getParsedMessageContents(text, message) {
	...
	contents = contents.replace(
		/^<div data-server-rendered="true">([^]+)<\/div>$/m,
		"$1"
	);
...
describe("parse Handlebars helper", () => {
	it("should not introduce xss", () => {
		...
		const actual = testCases.map((testCase) =>
			getParsedMessageContents(testCase.input)
		);
	...
	[
		...
		},
		{
			...
			expected:
				'<span style="color:#415364;background-color:#FF9E18;">The Lounge</span>',
		},
		{
			name: "italic",
			input: "\x1ditalic",
	...
	[
		...
		},
		{
			...
			input: "🤦🏿‍♀️",
			expected:
				'<span role="img" aria-label="Emoji: woman facepalming: dark skin tone" title="Emoji: woman facepalming: dark skin tone" class="emoji">🤦🏿‍♀️</span>',
		},
		{
			name: "with modifiers",
			input: "🤷‍♀️",
